	...
	if(mainDiv != null) {
		...
		inData.forEach(function(inCurrent) {
			...
			box = box.replace(/THE_ID/g, cbId)
					 ...
					 .replace(/THE_TITLE/g, title + " (" + inCurrent.count + ")");
			mainDiv.append(box);
...
function doTheQuery() {
	...
	jQuery('.oneCheckbox:checkbox:checked').each(function(){
		cbQuery += " OR "
		switch(this.name) {
		case "events":
	...
	// Remove first OR
	if(cbQuery == "") {
		// OK, all unchecked => make sure we find nothing
		cbQuery = "ac:event = '" + Math.random() + "'";
	} else {
		cbQuery = cbQuery.replace("OR", "");
	}
